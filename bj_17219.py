...
# (조건)
# 첫째 줄에는 저장된 사이트 주소의 개수 N(1<=N<=100,000)과
...
def search_PW(storage: dict, key: str) -> str:
    ...
    if not storage.get(key):
        return input(
            'No password. Do you generate and store new random password? (y/n)'
        ).lower()
    return storage.get(key)


def gen_random_PW(num: int) -> str:
    '''generates password mixed by upper and lower letters
    and returns random password expressed string type'''
    length = num
...
def main():
    ...
    storage = {'www.abc.kr': 'ABCKR', 'www.korea.kr': 'KOREA', 'www.coding.kr': 'CODING', 'www.computer.kr': 'COMKR', 'www.Sam-Sung.kr': 'SAM'}
