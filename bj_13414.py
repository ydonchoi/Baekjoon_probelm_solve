...
# 대기목록에서 가장 앞에 있는 학생부터 자동으로 수강신청이 완료되며,
# 수강 가능 인원이 꽉찰 경우 나머지 대기 목록은 무시하고 수강신청을 종료한다.
# 입력 데이터는 표준 입력을 사용한다. 입력은 1개의 테스트 데이터로 구성된다.
# 첫째 줄에는 과목 수강 가능 인원 k(1<=k<=100,000)와
# 학생들이 버튼을 클릭한 순서로 기록한 대기목록 길이 L(1<=L<=500,000)이 주어진다.
# 둘째 줄부터는 수강신청 버튼을 클릭한 학생의 학년이 클릭 순서대로 주어진다.
# 학번은 8자리의 숫자로 이루어져 있다.
# 출력은 표준 출력을 사용한다.
# 수강신청 관리시스템 규칙 적용 이후 수강신청 성공 인원의 학번을 한 줄에 하나씩 출력한다.
#########################
...
def button_click(student_id: list, wait: list) -> list:
    ...
    while True:
        # 수강신청 버튼 입력 상황 가정
        press_button = int(input('Press 1 if participate, Press 0 if stop.'))
        button_count += 1
