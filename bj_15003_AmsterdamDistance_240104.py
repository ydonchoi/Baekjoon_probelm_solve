#파이썬 #백준 #문제풀이 #15003번 #AmsterdamDistance #최영돈(240104~240105, band.us@ydonchoi)
'''
(문제)
암스테르담 거리는 반지름이 r인 반원 형태의 길로 구성되어 있다.
암스테르담 거리 한 곳(A)에서 다른 곳(B)으로 이동하는 최단 거리를 구하라.
첫번째 줄에는 세 정수(M, N, R)가 주어진다.
- R(실수): 전체 반원의 반지름 길이
- M(정수): 반원의 중앙에서 세로로 구획하는 구획 수
- N(정수): 반원의 중앙으로부터 가로로 구획하는 구획 수
두번째 줄에는 A 장소(n1,m1)와 B 장소(n2,m2)가 차례대로 주어진다.
이때, A에서 B로 가는 최단 거리를 구하시오.
'''
# solution
...
def main():
    ...
    if n1 <= (N+1) and n2 <= (N+1) and m1 <= (M+1) and m2 <= (M+1):
        print(M,N,R,'\n',m1,n1,m2,n2)
        AmsterdamDistance(M,N,R,m1,n1,m2,n2)
    else:
